// Conference data types and data
// Ported from js/conferences-data.js
...
export interface Conference {
    ...
    secGenAccounts: string;
    parliamentarianAccounts: string;
    pricePerDelegate: string;
    independentDelsWelcome: boolean;
    independentSignupLink: string;
    advisorSignupLink: string;
    disabledSuitable: boolean;
    sensorySuitable: boolean;
    committees: string[];
    uniqueTopics: string[];
    chairsPages: string;
    allocations: string[];
...
export const MUN_CONFERENCES_DATA: Conference[] = [
    {
        id: 1,
        name: "MUN07 IV",
...
// Prospective conferences (not yet fully registered)
...
export function getAllCommittees(): string[] {
    ...
    MUN_CONFERENCES_DATA.forEach(conf => {
        conf.committees.forEach(committee => {
            // Extract committee abbreviation (first word before " - ")
            const match = committee.match(/^([A-Z]+)/);
            if (match && match[1]) {
